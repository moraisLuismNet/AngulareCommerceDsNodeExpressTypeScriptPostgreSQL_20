...
export class GenresService {
    ...
    return new Observable<IGenre>(subscriber => {
      this.http.put<any>(
        ...
        next: (response) => {
          // Handle different response formats
          const result: IGenre = {
            IdMusicGenre: response.IdMusicGenre || response.idMusicGenre || genre.IdMusicGenre,
  ...
  deleteGenre(idMusicGenre: number): Observable<IGenre> {
    ...
    return new Observable<IGenre>(subscriber => {
      this.http.delete<any>(`${this.urlAPI}music-genres/${idMusicGenre}`, { headers }).subscribe({
        next: (response) => {
          // Handle different response formats
          const result: IGenre = {
